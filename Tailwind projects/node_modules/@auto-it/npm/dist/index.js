...
class NPMPlugin {
    ...
    apply(auto) {
        ...
        if (((_b = auto.config) === null || _b === void 0 ? void 0 : _b.versionBranches) && branch) {
            isMaintenanceBranch = branch.includes(typeof auto.config.versionBranches === "boolean"
                ? "version-"
        ...
        auto.hooks.beforeCommitChangelog.tapPromise(this.name, async ({ commits, bump, useVersion }) => {
            ...
            await lernaPackages.reduce(async (last, lernaPackage) => {
                ...
                const title = `v${useVersion || semver_1.inc(lernaPackage.version, bump)}`;
        ...
        auto.hooks.version.tapPromise(this.name, async ({ bump, useVersion, dryRun, quiet }) => {
            ...
            const latestBump = isBaseBranch
                ? useVersion ||
                    (await bumpLatest(await package_json_utils_1.loadPackageJson(), bump)) ||
                    bump
                : useVersion || bump;
            if (dryRun) {
